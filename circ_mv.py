'''
circ_mv.py
描述：从tushare上抓取流通资本，需安装tushare包
'''
...
code=list(pro.daily_basic(ts_code='',trade_date='20190909',fileds='circ_mv')['ts_code'])       #获取该交易日所有股票代码

#print(OutCap)
for i in code:
    s = pd.DataFrame(pro.daily_basic(ts_code=i,start_date='20090909',end_date='20190909'))
    s=s[['trade_date', 'circ_mv']]                                                              #获取两列，第一列为日期，第二列为流通资本
    s.columns=['trade_date',i]                                                                  #将流通资本列名改为股票代码
    OutCap=OutCap.merge(s,left_on='cal_date',right_on='trade_date',how='outer')
    OutCap=OutCap.drop(['trade_date'],axis=1)
    print(OutCap)

OutCap.to_excel('circ_mv.xlsx')                                                                 #导出至excel文件